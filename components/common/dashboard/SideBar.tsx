...
import { dashboardMenuLinks, dashboardSystemLinks } from '@/lib/route-links';

const SideBar = () => {
  const location = usePathname();

  const toggleSubList = (isOpen: boolean, index: number) => {
    const hasOpenItem = dashboardMenuLinks.find((item) => item.id === index);

    if (hasOpenItem) {
      hasOpenItem.isOpen = isOpen === false ? true : false;
    }

    console.log(hasOpenItem);
  };
            ...
            {dashboardMenuLinks.map((item, index) => (
              <li key={item.id}>
                ...
                >
                  ...
                  {item.sub_link && (
                    <ChevronRight
                      className={`text-(--primary) transition-all ease-in-out ${item.isOpen ? 'rotate-90' : 'rotate-0'}`}
                    />
                  )}
                </Link>
                ...
                >
                  ...
                  <ul className="overflow-hidden flex flex-col gap-2 pl-8">
                    {item.sub_link?.map((subLinkItem, index) => (
                      <li key={index}>
                        <Link
          ...
          <ul className="flex-1">
            {dashboardSystemLinks.map((item) => (
              <li key={item.id}>
                <Link
                  href={item.path}
                  className={`flex items-center align-middle gap-4 p-3 rounded-xl w-full hover:bg-[#F7F5FF] transition-all ease-in-out ${
